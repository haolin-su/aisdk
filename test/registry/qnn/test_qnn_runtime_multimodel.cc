...
int ParseAndCheckArgs(int argc, const char *argv[], CommandParser &options)
{
    ...
    options.AddOption("--qnn_modeldlc_file", "", "QNN model dlc file path");
...
int main(int argc, const char* argv[]) {
    ...
    printf("qnn_modeldlc_file: %s\n", qnn_modeldlc_file.c_str());
    printf("input_file: %s\n", input_file.c_str());
    ...
    printf("Successfully loaded QNN function pointers.\n");

    char* backendBuildId{nullptr};
    if (QNN_SUCCESS !=
        qnnFunctionPointers.qnnInterface.backendGetBuildId((const char**)&backendBuildId)) {
        printf("Unable to get build Id from the backend.\n");
    }
    printf("Backend build Id: %s\n", backendBuildId);
    ...
    ret = Init(&qnnFunctionPointers, logHandle, backendHandle, deviceHandle, context, contextConfigs, 
                graphHandle, graphsInfo, graphsCount, profileBackendHandle, cache_model_file.c_str(), qnn_modeldlc_file.c_str(), false);
    if (ret != 0) {
    ...
    ret = ready_tensors(&input_tensors, &output_tensors, inputNum, outputNum, inputSize, outputSize, input_dataType, output_dataType, 
                        inputDataSize, outputDataSize, graphHandle, context, graphsInfo, graphsCount);

    // 塞数据到inputs
    // 1. 读取输入文件
    ret = read_input_data(input_tensors, inputNum, inputDataSize, input_file.c_str());
    if (ret != 0) {
    ...
    if (QNN_GRAPH_NO_ERROR != status) {
        printf("Failed to execute graph, error code: %ld\n", status);
        return -1;
    }
    ...
    printf("Write output data to file: %s\n", output_file.c_str());
