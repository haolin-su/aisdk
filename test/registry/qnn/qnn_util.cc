...
int load_qnn_function_pointers(QnnFunctionPointers* qnnFunctionPointers, const char* qnn_backend_file, 
    ...
    // 如果qnn_modeldlc_file为空，则使用QnnModel.so的接口
    if (strcmp(qnn_modeldlc_file, "") == 0) {
        ...
        std::string modelDlcFunc = "QnnModel_composeGraphsFromDlc";
        ...
        if (nullptr == qnnFunctionPointers->qnnModelComposeGraphsFromDlc) {
            printf("Failed to resolve %s %p, %s function in modeldlc library.\n", qnn_modeldlc_file, libmodelDlcHandle, modelDlcFunc.c_str());
            return -1;
        }
        printf("Found QnnModel_composeGraphsFromDlc function in model library.\n");
    }
             ...
             const char* dlc_model_file, bool used_cache)
{
    ...
    if (QNN_PROPERTY_ERROR_UNKNOWN_KEY == qnnStatus) {
        ...
        Qnn_ErrorHandle_t qnnStatus = qnnFunctionPointers->qnnInterface.deviceCreate(logHandle, nullptr, &deviceHandle);
...
// {
    ...
    // GraphInfo_t** graphsInfo = *graphsInfos;
    ...
    if (!used_cache){
        ...
        if (qnnFunctionPointers->qnnModelComposeGraphsFromDlc == nullptr) {
            auto ret = qnnFunctionPointers->composeGraphsFnHandle(backendHandle,
                                                                ...
                                                                logStdoutCallback,
                                                                logLevel);
            if (ModelError_t::MODEL_NO_ERROR != ret ) {
                printf("Failed in composeGraphs(), error code: %d.\n", ret);
                return -1;
            }
            printf("Output num:%d\n", graphsInfo[0]->numOutputTensors);
        } else {
            ...
            // GraphConfigInfo_t **graphConfigsInfo = nullptr;
            printf("Loading model from dlc file: %s\n", dlc_model_file);
        ...
        Qnn_ErrorHandle_t ret = qnnFunctionPointers->qnnInterface.contextGetBinarySize(context, &requiredBufferSize);
        if (QNN_CONTEXT_NO_ERROR != ret) {
            printf("Could not get the required binary size, error code: %ld.\n", ret);
            return -1;
        }
                  ...
                  GraphInfo_t** graphsInfo, uint32_t &graphsCount) 
{
    ...
    printf("inputTensorCount: %d, outputTensorCount: %d\n", inputTensorCount, outputTensorCount);
    inputNum = inputTensorCount;
    outputNum = outputTensorCount;
    ...
    *output_tensors =  (Qnn_Tensor_t*)calloc(1, outputTensorCount * sizeof(Qnn_Tensor_t));
    ...
    for (size_t i = 0; i < outputTensorCount; i++) {
        ...
        std::vector<int> outputDims;
        outputSize = 1;
        for (size_t j = 0; j < (*graphsInfo)[graphIdx].outputTensors[i].v2.rank; j++) {
            outputDims.push_back((*graphsInfo)[graphIdx].outputTensors[i].v2.dimensions[j]);
            outputSize *= (*graphsInfo)[graphIdx].outputTensors[i].v2.dimensions[j];
        }
        Qnn_ClientBuffer_t clientBuffer = QNN_CLIENT_BUFFER_INIT;
...
int read_input_data(Qnn_Tensor_t* input_tensors, uint32_t inputNum, uint32_t inputDataSize, const char* input_file) {
    // 读取输入文件
    ...
    for (size_t i = 0; i < inputNum; i++) {
        memcpy((char *)QNN_TENSOR_GET_CLIENT_BUF(input_tensors[i]).data, &inputData[i], inputFileSize);
    }
