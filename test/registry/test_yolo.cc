#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <vector>
#include <unordered_map>
#include <memory>
#include <iostream>
#include <fstream>
#include <cstring>
#include <cstdlib>
#include <iterator>
#include <algorithm>
#include <opencv2/opencv.hpp>
#include <opencv2/dnn.hpp>
#include <opencv2/imgproc.hpp>
#include <iostream>

#include "DlSystem/UserBufferMap.hpp"
#include "DlSystem/IUserBuffer.hpp"
#include "DlSystem/DlEnums.hpp"
#include "DlSystem/RuntimeList.hpp"
#include "DlContainer/IDlContainer.hpp"

#include "SNPE/SNPE.hpp"
#include "SNPE/SNPEBuilder.hpp"
#include "SNPE/SNPEFactory.hpp"

#include "registry/algorithm/cv_detection_yolo_postproc.h"
#include "registry/algorithm/cv_segmentation_yolo_postproc.h"
#include "registry/algorithm/utils/postproc_opencv.h"
#include "core/tensor/tensor.h"
#include "core/utils.h"

void read_tensor_raw(std::string& file_path, std::vector<int>&input_shape, std::shared_ptr<aisdk::Tensor>& tensor)
{
    // 设定矩阵的尺寸和类型
    int batchs = input_shape[0]; // 批次数
    int channels = input_shape[1]; // 通道数
    int height = input_shape[2]; // 高度
    int width = input_shape[3];  // 宽度

    // 创建一个 cv::Mat 以存储读取的数据
    cv::Mat input_tensor(height, width, CV_32FC3); // 3 通道浮点型

    // 打开 .raw 文件以二进制方式读取
    std::ifstream file(file_path, std::ios::binary);
    if (!file.is_open()) {
        std::cerr << "Error opening " << file_path << std::endl;
        return; // 错误处理
    }

    // 读取数据
    file.read(reinterpret_cast<char*>(input_tensor.data), input_tensor.total() * input_tensor.elemSize());
    
    // 检查读取是否成功
    if (!file) {
        std::cerr << "Error reading from " << file_path << std::endl;
        return; // 错误处理
    }

    file.close(); // 关闭文件

    tensor->SetData(input_tensor.data);
    aisdk::TensorShape tensor_shape{4, {batchs, height, width, channels}};
    tensor->SetShape(tensor_shape);
}

void create_mat_from_raw(std::string& file_path, std::vector<int>&input_shape, cv::Mat& mat)
{
    // 打开 .raw 文件以二进制方式读取
    std::ifstream file(file_path, std::ios::binary);
    if (!file.is_open()) {
        std::cerr << "Error opening " << file_path << std::endl;
        return; // 错误处理
    }

    size_t total_size = 1 * sizeof(float);
    int count = 0;
    for (auto i : input_shape) {
        total_size *= i;
        count++;
    }

    std::vector<float> buffer(total_size / sizeof(float)); // 创建缓冲区
    file.read(reinterpret_cast<char*>(buffer.data()), total_size);

    if (!file) {
        std::cerr << "Error reading from " << file_path << std::endl;
        return; // 错误处理
    }

    file.close(); // 关闭文件

    mat = cv::Mat(count, input_shape.data(), CV_32F, buffer.data()).clone();

    // mats.clear(); // 清空之前的 Mat
    // for (int b = 0; b < batchs; ++b) {
    //     cv::Mat mat(height, width, CV_32FC(channels), buffer.data() + (b * height * width * channels));
    //     mats.push_back(mat.clone()); // 使用 clone 复制数据
    // }
}

void test_1(){
    std::cout << "test 1\t" << "YoloDetectionPostprocOpencv" << std::endl;
    std::string file_path = "outputs_0.raw";
    std::vector<int> input_shape = {1, 116, 8400};
    cv::Mat mat(3, input_shape.data(), CV_32F);  // 3维的浮点型矩阵

    create_mat_from_raw(file_path, input_shape, mat);

    cv::Mat out_mat;

    int ret = aisdk::YoloDetectionPostprocOpencv(mat, out_mat, 0.8);
    std::cout << "ret: " << ret << std::endl;
}


void test_2(){
    std::cout << "test 1\t" << "YoloSegmentationPostprocOpencv" << std::endl;
    std::string file_path_1 = "outputs_0.raw";
    std::string file_path_2 = "outputs_1.raw";
    std::vector<int> input_shape_1 = {1, 116, 8400};
    cv::Mat mat_1(3, input_shape_1.data(), CV_32F);  // 3维的浮点型矩阵

    std::vector<int> input_shape_2 = {1, 32, 160, 160};
    cv::Mat mat_2(4, input_shape_2.data(), CV_32F);  // 4维的浮点型矩阵

    create_mat_from_raw(file_path_1, input_shape_1, mat_1);
    create_mat_from_raw(file_path_2, input_shape_2, mat_2);

    cv::Mat out_mat_1;
    cv::Mat out_mat_2;

    int ret = aisdk::YoloSegmentationPostprocOpencv(mat_1, mat_2, out_mat_1, out_mat_2, 0.5, 32);   
    std::cout << "ret: " << ret << std::endl;
}



int main(int argc, char* argv[]) {
    std::cout << "test yolo" << std::endl;
    //test_1();
    test_2();
#if 0  //这是业务测试
    aisdk::YoloPostProcNode yolo_postproc_node;
    aisdk::YoloSegPostProcNode yolo_seg_postproc_node;
    yolo_postproc_node.Init("config");
    std::shared_ptr<aisdk::Tensor> input_tensor = std::make_shared<aisdk::Tensor>();
    std::shared_ptr<aisdk::Tensor> output_tensor = std::make_shared<aisdk::Tensor>();

    std::string file_path = "input_tensor.raw";
    std::vector<int> input_shape = {3, 640, 640};
    read_tensor_raw(file_path, input_shape, input_tensor);

    yolo_postproc_node.Process(input_tensor, output_tensor);
    std::vector<std::shared_ptr<aisdk::Tensor>> input_tensors;
    std::vector<std::shared_ptr<aisdk::Tensor>> output_tensors;
    input_tensors.push_back(input_tensor);
    input_tensors.push_back(input_tensor);
    output_tensors.push_back(output_tensor);
    output_tensors.push_back(output_tensor);
    yolo_seg_postproc_node.Process(input_tensors, output_tensors);
#endif
    return 0;
}