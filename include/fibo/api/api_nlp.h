...
#include <vector>
#include <functional>
#include "fibo/model/model_nlp.h"
...
public:
    ...
    // 返回值：0表示成功，其他表示失败
    int GenerateSync(std::vector<int>& text, result::ResultNLP_Text& result, int timeout = 0);
    ...
    // 参数说明：
    // 返回值：模型信息结构体
     int GetModelInfo(FIBO_NLP_Model &model_info);
