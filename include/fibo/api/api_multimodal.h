...
// 初始化参数
    ...
    // 返回值：0表示成功，其他表示失败
    int GenerateSync(std::string& text, std::string& img_path, result::ResultMultimodal& result);
    ...
    //  result：任务响应，非必须，从中获取值
    // 返回值：0表示成功，其他表示失败
    int GenerateSync(std::string& text, const FIBO_CV_Img& img, result::ResultMultimodal& result);
    ...
    //  tag：任务标签，非必须
    // 返回值：0表示成功，其他表示失败
    int GenerateAsync(std::string& text, std::string& img_path, MultimodalInferAsyncCallback cb, int timeout = 0, std::string tag = "");
    ...
    //  tag：任务标签，非必须
    // 返回值：0表示成功，其他表示失败
    int GenerateAsync(std::string& text, const FIBO_CV_Img& img, MultimodalInferAsyncCallback cb, int timeout = 0, std::string tag = "");
