...
#include <vector>
#include <functional>
#include "fibo/model/model_audio.h"
...
public:
    ...
    //  result 是转录的结果
    //  timeout 超时时间，单位秒，非必须，默认0，表示不超时
    // 返回值：错误码
    int transcribeSync(FIBO_Audio audio, result::ResultNLP_Audio_Transalte &result, int timeout = 0);
    ...
    //  result 是转录的结果
    // 返回值：错误码
    int transcribeAsync(FIBO_Audio audio , AudioInferAsyncCallback cb, int timeout = 0, const std::string tag = "");

    // 获取模型信息
    // 功能说明：获取模型信息
    // 参数说明：
    // 返回值：模型信息结构体
    int get_model_info(FIBO_Audio_Model &model_info);
