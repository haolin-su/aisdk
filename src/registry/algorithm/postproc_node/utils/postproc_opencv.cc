...
namespace aisdk
{
    ...
    {
        // no detections, return empty indices
        if(boxes.empty()){
            indices.clear();
        ...
        int32_t max_boxes = std::min(static_cast<int>(indices.size()), MAX_NUM_OBJECTS);
    ...
    int32_t ExtractBox(const cv::Mat& output, std::vector<int32_t>& box)
    {
        ...
        box.push_back(static_cast<int32_t>(output.at<float>(0)));
        box.push_back(static_cast<int32_t>(output.at<float>(1)));
        box.push_back(static_cast<int32_t>(output.at<float>(2)));
        box.push_back(static_cast<int32_t>(output.at<float>(3)));
    ...
    int32_t ProcessDetectionPredictions(const cv::Mat& results, float confidence_threshold, Predictions& predictions)
    {
        ...
        for (int32_t i = 0; i < output.rows; ++i)
        {
            ...
            if (max_score >= confidence_threshold) {
                ...
                if(ExtractBox(output.row(i), box) != ErrorCode::ERROR_CODE_OK){
    ...
    {
        if (predictions.class_ids.empty() || predictions.scores.empty() || predictions.boxes.empty() || indices.empty()){
            return ErrorCode::ERROR_CODE_CV_INVALID_PARAM;
        }

        // each row: class_id, score, bbox(x, y, w, h）
        output.create(std::vector<int32_t>(static_cast<int32_t>(indices.size()), 6), CV_32FC1);

        for (int32_t i = 0; i < indices.size(); ++i) {
            int32_t index = indices[i];
            output.at<int32_t>(i, 0) = static_cast<int32_t>(predictions.class_ids[index]);
            output.at<float>(i, 1) = static_cast<float>(predictions.scores[index]);
            
            const auto& box = predictions.boxes[index];
            output.at<int32_t>(i, 2) = static_cast<int32_t>(box[0]);
            output.at<int32_t>(i, 3) = static_cast<int32_t>(box[1]);
            output.at<int32_t>(i, 4) = static_cast<int32_t>(box[2]);
            output.at<int32_t>(i, 5) = static_cast<int32_t>(box[3]);
        }
        return ErrorCode::ERROR_CODE_OK;
    }
    ...
    {
        if(results.empty() || confidence_threshold < 0.0 || confidence_threshold > 1.0 || num_masks < 0){
            return ErrorCode::ERROR_CODE_CV_INVALID_PARAM;
        }
    ...
    int32_t CropMask(cv::Mat& mask_map, const std::vector<std::vector<int32_t>>& scale_boxes, const std::vector<std::vector<int32_t>>& boxes,
                    const cv::Mat& masks, const int32_t input_height, const int32_t input_width, const int32_t output_height, const int32_t output_width) {

        if(scale_boxes.empty() || boxes.empty() || masks.empty() || input_height <= 0 || input_width <= 0 || output_height <= 0 || output_width <= 0) {
            return ErrorCode::ERROR_CODE_CV_INVALID_PARAM;
        }
        ...
        // here should change the boxes coordinates from (x, y, w, h) to (x1, y1, x2, y2) format
    ...
    int32_t ProcessSegmentationMaskPredictions(const cv::Mat& results, const cv::Mat& mask_predictions, std::vector<std::vector<int32_t>> boxes, cv::Mat& mask_map)
    {
        ...
        // TODO: 这里的img_height和img_width需要从算法的信息获取，暂时无可获取的接口或设计，现在先设定为模型的input尺寸
    ...
    int32_t YoloSegmentationPostprocOpencv(const cv::Mat& prediction_input, const cv::Mat& mask_input, cv::Mat& prediction_output, cv::Mat& mask_output, float confidence_threshold, int32_t num_masks)
    {
        if(prediction_input.empty() || mask_input.empty() || 
            confidence_threshold < 0.0 || confidence_threshold > 1.0 || num_masks < 0){
            return ErrorCode::ERROR_CODE_CV_INVALID_PARAM;
        }
        ...
        // calculate the right memory size for stroing the mask predictions
        ...
        if (totalRows == 0) {
            prediction_output.release();
            mask_output.release();
            return ErrorCode::ERROR_CODE_OTHER_ERROR;
        }
        ...
        // TODO:for debug, delete these
        // std::string prediction_output_file_name = "prediction_output.raw";
        // std::string mask_output_file_name = "mask_output.raw";
        // SaveMatAsRaw(prediction_output, prediction_output_file_name);
        // SaveMatAsRaw(mask_output, mask_output_file_name);
