#ifndef FIBO_CV_SEGMENTATION_YOLO_PREPROC_H_
#define FIBO_CV_SEGMENTATION_YOLO_PREPROC_H_

#include "core/node/node.h"

namespace aisdk {

class YoloSegPreProcNode : public INode {
public:
    YoloSegPreProcNode() = default;
    ~YoloSegPreProcNode() = default;
    // INode() : INode(""), type_(NODE_TYPE_INPUT) {}
    YoloSegPreProcNode(const std::string& name) : INode(name) {}
...
// register_preprocess("PreProcess_Yolo_any", YoloSegPreProcNode, params_str);

} // namespace aisdk

#endif // FIBO_CV_SEGMENTATION_YOLO_PREPROC_H_