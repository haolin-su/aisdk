...
public:
    ...
    // 处理数据
    bool Process(TensorPtr& input, TensorPtr& output) override ;
    bool Process(std::vector<TensorPtr>& inputs, std::vector<TensorPtr>& outputs) override ;
    ...
    // 获取硬件IP
    // 获取输入输出tensor
    int GetInputTensor(const std::string& key, TensorPtr& tensor) override ;
    int GetOutputTensor(const std::string& key, TensorPtr& tensor) override ;

    // 设置输入输出tensor
    int SetInputTensor(const std::string& key, TensorPtr& tensor) override ;
    int SetOutputTensor(const std::string& key, TensorPtr& tensor) override ;
